...
SAVE_DIR = SAVE_BASEPATH+'all_channels_filt_eegs/'
...
def eeg_from_parquet(parquet_path: str, display: bool = False) -> np.ndarray:
    ...
    eeg_feature =  ['Fp1', 'F3', 'C3', 'P3', 'F7', 'T3', 'T5', 'O1', 'Fz', 'Cz', 'Pz', 'Fp2', 'F4', 'C4', 'P4', 'F8', 'T4', 'T6', 'O2', 'EKG']
    
    # === Extract middle 50 seconds ===
    eeg = pd.read_parquet(parquet_path, columns=eeg_feature)
    rows = len(eeg)
    ...
    if display: 
        ...
        offset = 0
    # === Convert to numpy ===
    data = np.zeros((10_000, len(eeg_feature))) # create placeholder of same shape with zeros
    for index, feature in enumerate(eeg_feature):
...
def main():
    ...
    for i, eeg_id in tqdm(enumerate(eeg_ids)):  
        ...
        all_eegs_copy[eeg_id] = data

    eeg_feature =  ['Fp1', 'F3', 'C3', 'P3', 'F7', 'T3', 'T5', 'O1', 'Fz', 'Cz', 'Pz', 'Fp2', 'F4', 'C4', 'P4', 'F8', 'T4', 'T6', 'O2', 'EKG']

    ## preprocess the eeg data
    all_filt_eegs = dict()
    eeg_channel_names = eeg_feature # this is using all the features
    
    #Preprocessing EEG data
    x = ['eeg']*19 # len(eeg_features) - 1
    x.append('ecg')
    ...
    for k in list(all_eegs_copy.keys())[start_count:]:
        ...
        if (start_count+1) % chunk_size == 0:
            ...
            np.save(f'{SAVE_DIR}all_channels_filt_eegs{chunk_num_str}.npy', all_filt_eegs)
